...
//
// Пять кейсов существуют, главная показывает три: «Этот сайт»,
...
// опубликованы вместе с возвратом честных ссылок. До этого «ИИ-консультант»
// и «Заявка-Хаб» стояли на главной вторым и третьим блоками (D-048,
// `04-cases-brief.md`, раздел 2.1), а до 2026-08-19 там же была «Фабрика
// ботов» — её владелец снял из данных целиком. Ещё один недостроенный кейс
// снят тем же способом правкой владельца 2026-08-26.
//
// Разница между такими снятиями и «ИИ-консультантом» с «Заявкой-Хабом»
// важна и выражена в данных: у снятых целиком кейсов здесь нет НИЧЕГО, а
// «ИИ-консультант» и «Заявка-Хаб» остаются записями с `onHome: false` —
// вместе с описанием и стеком, потому что их текст утверждён владельцем
// построчно (`02-texts.md`, секция 5) и его же возьмёт страница каталога.
// Их рисунки (`CaseFlowIllustration.astro`, `CaseDialogueIllustration.astro`)
// по той же причине остаются в коде: они перестали выводиться, а не исчезли.
//
// Правило чередования сторон (`homeOrder % 2 === 0`) от состава не меняется:
// первый блок обычный, второй зеркальный.
//
// Storefront возвращён на главную отдельной правкой владельца 2026-08-24:
// это второй полноширинный блок с тремя подтверждёнными вариантами витрины
// — Mariosa, Zayac и Yasmina.
//
...
export interface CaseCard {
  ...
  /** Заголовок карточки. */
  title: string;
  /** Описание. У записи без утверждённого текста его нет: она не выходит в
   *  публичный каталог и по рыбе не дописывается (план 02-home-plan.md,
   *  задача 1). */
  description?: string;
 ...
 *  запись, у которой уже подтверждены и описание, и стек. Так запись без
 *  утверждённого текста не превращается в пустую SEO-заглушку и не
 *  обрастает текстом по догадке.
 *
 *  Условие вынесено в именованную функцию правкой `2026-08-26`, и вот зачем.
 *  Со снятием SlotBook (решение владельца того же дня) в `CASES` не осталось
 *  НИ ОДНОЙ неполной записи: все пять оставшихся несут и описание, и стек.
 *  Значит отрицательная ветка этой границы — та самая, ради которой она
 *  заведена, — перестала исполняться хоть где-нибудь: ни в данных, ни в
 *  тестах. Предохранитель остался в коде и молча перестал быть доказанным,
 *  а это ровно та форма, в какой сторожа этого репозитория уже засыпали.
 *
 *  Инлайновое условие проверить синтетической записью было нельзя, не трогая
 *  сигнатуру `publishedCases()`. Отдельная функция позволяет предъявить ей
 *  неполную запись прямо в тесте, ничего не подмешивая в боевые данные. */
export function isPublishable(item: CaseCard): item is PublishedCase {
  return Boolean(item.description) && Boolean(item.stack);
}

export function publishedCases(locale: Locale = 'ru'): PublishedCase[] {
  return cases(locale).filter(isPublishable);
}
