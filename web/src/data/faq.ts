...
// сама. На главной вопросы остаются обычным видимым контентом: коммерческое
// портфолио не относится к типам сайтов, для которых Google показывает FAQ
// rich results, поэтому дублировать секцию в `FAQPage` здесь не нужно.
//
