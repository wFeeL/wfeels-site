...
const AI_CONSULTANT_SPREADS: readonly CaseSpread[] = [
    ...
    images: [
      {
        ...
        alt: 'Виджет консультанта отвечает на семь вопросов о процедурах, противопоказаниях и отмене записи со ссылками на источники, а на вопрос о погоде в Москве отвечает, что не нашел этого в материалах салона, и вместо источника явно показывает, что источников нет',
  ...
  },
  {
    ...
    kind: 'photo',
    /* Кадр обрезан до содержательного фрагмента 1586×480 (был ландшафт
     * 1586×992): нижние ~56% исходного снимка — пустое поле редактора,
     * последняя строка текста заканчивалась на y≈436 из 992. На ширине 390
     * читатель получал тёмное пятно, где содержательного даже меньше
     * половины кадра, — обрезка снимает балласт, а не читаемость (очередь
     * `70-workshop/plans/site-v3/04-queue-2026-08-26.md`, пункт 3). Решение
     * владельца — «обрезать до фрагмента», не заводить второй файл под
     * мобильную раскладку и не переснимать разворот с мобильного экрана
     * админки (обоснование там же). `frameWidth`/`frameHeight` заданы явно,
     * иначе резервирование места под кадр (раздел 3.5 брифа) использует
     * умолчание 1586×992 и не совпадёт с настоящим файлом. */
    frameWidth: 1586,
    frameHeight: 480,
    images: [
      {
