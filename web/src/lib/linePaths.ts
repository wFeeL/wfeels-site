...
export interface LinePathEntry {
   ...
   *  branch">`; на главной их две» — здесь заведено поле только под
   *  первую (`process`, «спина шагов»); вторая (`cases`, отвод к «Замеру»)
   *  — отдельная задача, этим полем НЕ занята. Раскрывается ТОЙ ЖЕ
   *  шторкой, что и `wide` (`reveal`/`overhangPercent` выше не
   *  дублируются под неё): шторка закрывает содержимое коробки целиком,
   *  второй путь внутри неё дорисовывается синхронно без единой новой
   *  строки механики. Необязательное поле — секция без ветви его просто
   *  не несёт. */
  branch?: string;
}
...
const HAND_DRAWN: Readonly<Record<string, LinePathEntry>> = {
  ...
  // services — раздел 10.4 брифа `11-line-narrator-brief.md` (правка
  // 2026-08-27): «прямая, левый причал» — событие «траверс слева направо»
  // отсюда СНЯТО и переехало в `pricing`. Причина: на 1440 краска траверса
  // здесь целиком тонет под сеткой карточек услуг (76% высоты секции, 89%
  // ширины холста, просвет 20 px при краске 47 px) — владелец увидел это как
  // разрыв линии. Секция становится спокойным прогоном, продолжающим левый
  // док, на котором закончился `pain`.
  services: (() => {
    const h = vbHOf('services');
    return buildEntry(h, straightPath(h, DOCK_LEFT), narrowPath(h));
  })(),
  // pricing — раздел 10.4 брифа `11-line-narrator-brief.md`: «траверс слева
  // направо» — событие переехало сюда из `services`, чтобы краска средней
  // линии физически прошла сквозь карточку «Корпоративный сайт»
  // (`#pricing .top-grid > .card--accent`, 543…897 × 3462…3911 на 1440):
  // вход через левую кромку, выход через нижнюю, 284 px из 449 px высоты
  // карточки, углы пересечения 44°/46° (≥ 30°, Г-5). Выходит на тот же
  // правый док, на котором заканчивался прежний прямой прогон, — сторона
  // и переход ниже `pricing` не меняются ни на секцию.
  pricing: (() => {
  ...
  // process — раздел 3 П4 брифа `11-line-narrator-brief.md`: «спина шагов»,
  // вторая волосяная нить в той же коробке, соединяющая цифры 01…05
  // («линия, которая соединяет их в одно целое, в конце уходя вправо
  // снизу» — дословно владелец). Опорная строка `d`, отправная точка, а
  // не догма (сам бриф, раздел 3 П4(б)): рождается ВНУТРИ краски средней
  // линии на правом доке (полоса vbY 100…160, до начала кубики траверса —
  // единственное место секции, где краска средней линии есть выше цифр,
  // Г-2 запрещает свободный круглый торец в воздухе), пологой S идёт к
  // оси x=88 (зазор 16 px до правого дока, 19 px до левой кромки цифр),
  // встаёт на неё не ниже vbY=260 (выше цифры 01, vbY=290), НЕ рисуется в
  // полосе vbY 800…900 (там её накрывает краска средней линии на высоте
  // цифры 04 — второй `M` открывает новый подпуть, разрыв), ниже vbY=1060
  // поворачивает вправо и уходит за viewBox на x=1000 в полосе vbY
  // 1075…1130 («в конце уходя вправо снизу» — дословно). Числа `vbY`
  // цифр (290/484/678/848/1042) и до/после разрыва (795/905) сверены с
  // переизмерением 2026-08-26 на `wt/casebrief` — раскладка ниже `cases`
  // уехала на +68px документа, но `vbY` считается от верха СВОЕЙ секции и
  // не изменился ни на единицу (врезка раздела 1 брифа).
  process: (() => {
    const h = vbHOf('process');
    return {
      ...buildEntry(h, traversePath(h, DOCK_RIGHT, DOCK_LEFT), narrowPath(h)),
      branch:
        'M941,120 C700,150 300,200 88,260 L88,795 ' +
        'M88,905 L88,1062 C88,1100 300,1120 1000,1120',
    };
  })(),
  ...
  // причала: 965 px проходит порог 850 (раздел 3, сноска Г-3). Заканчивает
