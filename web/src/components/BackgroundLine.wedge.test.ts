...
describe('линия на фоне — сторож клина первого экрана (раздел 12.8)', () => {
  ...
  it('шторка кроет голову: overhangPercent(hero)·vbH/100 ≥ |верхняя точка клина|', () => {
    const minY = Math.min(...points.map((p) => p.y));
    const curtainReachVb = (hero.overhangPercent * hero.vbH) / 100;
    ...
    expect(
      curtainReachVb,
      `шторка кроет только ${curtainReachVb.toFixed(2)} vb, а голова поднимается на ${Math.abs(minY).toFixed(2)} vb — overhangPercent считается не от полуширины головы`,
    ).toBeGreaterThanOrEqual(Math.abs(minY) - 0.01);
  });
});
