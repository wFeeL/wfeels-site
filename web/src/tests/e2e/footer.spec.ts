 ...
 * 4 px. Зазор несёт `gap` родителя (`.footer-meta`/`.meta-top`), а не поле
 * ссылки — раздел 10 пункт 2 запрещает трогать 14 (посчитано из метрики
 * шрифта под сторож 44 px).
 *
 * Решение З-4 меняет, ЧТО физически идёт следом за `.requisites`, и это не
 * одно и то же на всех трёх ширинах. На 390 и 768 `.meta-top` переносит
 * правую ячейку ПОД реквизиты (раздел 4.4: «ячейка снизу») — следующая цель
 * нажатия по-прежнему `.icon-links` внутри `.meta-aside`. На 1440
 * `.meta-top` держит их РЯДОМ (раздел 4.4: «ячейка рядом», контейнер
 * 1100/1180 против меры реквизитов 691,6 px), и правая ячейка короче левой
 * (80 px против 132) — значит физически под реквизитами на этой ширине
 * стоит уже `.legal-row`, тот же стык, что уже проверен на странице с
 * полосой (тест ниже). Три ширины — конкретные контрольные точки брифа: 390
 * (мобильный столбик), 768 (столбик), 1440 (ряд, дефект впервые измерен
 * здесь). */
for (const width of [390, 768] as const) {
  test(`зазор между реквизитами и следующей целью нажатия ≥ 16 px на ${width} px`, async ({ page }) => {
    ...
    await page.goto('/'); // на «/» полосы нет, ячейка снизу — следом идут значки (`.icon-links`)
...
test('зазор между реквизитами и юридическим рядом ≥ 16 px на 1440 px без полосы действия (ячейка рядом)', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto('/'); // на «/» полосы нет; на 1440 `.meta-aside` стоит РЯДОМ с реквизитами
                          // (короче их — 80 px против 132), а не под ними — под реквизитами
                          // физически идёт `.legal-row`.
  ...
  const gap = nextBox!.y - (lastBox!.y + lastBox!.height);
  expect(gap, 'зазор реквизиты → юридический ряд на 1440px без полосы').toBeGreaterThanOrEqual(16);
});
