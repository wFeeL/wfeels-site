...
test('ссылки подвала видно как ссылки', async ({ page }) => {
  ...
  // Правка дизайн-ревью 2026-08-19: ссылка подвала — пункт меню, не проза, и
  // цвет остаётся ЕДИНСТВЕННЫМ постоянным отличителем в покое; подчёркивание
  // теперь рисует `.link-label::after` только на `:hover`/`:focus-visible`
  // (доказано отдельным сторожем — `footer.spec.ts`, «подчёркивание живёт по
  // месту ссылки»). До правки здесь стояло обратное ожидание.
