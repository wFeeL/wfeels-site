 ...
 * воспроизведение замера на `astro preview` даёт медиану около 0,31 с — это
 * уже 22 % ниже напечатанного «0,4», то есть впритык к границе, и любой
 * ускорившийся прогон красил бы тест без всякой причины. */
...
test.describe('иллюстрация «Замер» — время загрузки не врёт', () => {
  test('медиана полной загрузки сходится с числом, напечатанным на рисунке', async ({ browser }) => {
    ...
    const note = (await page.locator('[data-cell="link"]').innerText()).trim();
