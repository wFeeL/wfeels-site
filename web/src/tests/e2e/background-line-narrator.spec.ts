...
test.describe('линия-рассказчик — зажигание цифр процесса (раздел 2 брифа `16-line-digits-and-finale-brief.md`; приёмка П-Ц1…П-Ц7)', () => {
  ...
  /* ПРАВКА `2026-08-28` (задача «карточка «Замер» на главной» — не эта
   *  секция, но геометрически связана): `CaseIllustrationField` перестал
   *  нести для иллюстрации «Замер» фиксированную высоту 384/416 px
   *  (`Cases.astro`) — поле теперь растёт под содержимое (222…237 px вместо
   *  384), и секция «Кейсы» (`#cases`, идёт сразу ПЕРЕД `#process` в
   *  порядке разметки, `data/sections.ts`) стала короче на ~95 px. Живой
   *  замер этим же файлом ДО той правки (комментарий выше, `2026-08-27`,
   *  порт 4601): `6305,6 · 6535,6 · 6764,6 · 6963,5 · 7193,6`. Живой замер
   *  ПОСЛЕ (`2026-08-28`, порт 4731, та же бисекция): `6211 · 6441 · 6670 ·
   *  6870 · 7099` — сдвиг ровно на укороченную высоту секции «Кейсы»
   *  (≈94–95 px), а не на произвольную величину: порядок, шаг между
   *  порогами (229 px) и запас П-Ц4 не изменились (см. прогон ниже).
   *  Числа брифа (`6338 · 6567 · …`, раздел 2.3 брифа `16-line-digits-and-
   *  finale-brief.md`) — чистая арифметика, посчитанная на СТАРОЙ высоте
   *  секции «Кейсы»; обновлять сам брифовый документ не в периметре этой
   *  задачи (база вне зоны записи фронтенд-исполнителя) — расхождение
   *  доложено в отчёте задачи, не подогнано молча (тот же принцип, что
   *  комментарий выше уже применяет к 31-32px разницы «арифметика vs
   *  живой замер»). Константы ниже — ЖИВОЙ замер, а не брифовые числа: тот
   *  же класс правки, что и предыдущие пять коммитов этого файла
   *  («Обновить … устаревших e2e-ассертов линии под принятую геометрию» —
   *  `fa2a960`, `8d08b26`, `7308b30`, `d40029a`, `4d197c9`). */
  const BRIEF_EXPECTED_1440 = [6211, 6441, 6670, 6870, 7099];
  const BRIEF_EXPECTED_1180 = [6173, 6402, 6631, 6831, 7060];
