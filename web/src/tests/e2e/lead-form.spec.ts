...
// Переключатель вернулся 2026-08-22 вместе с английской главной и стоит на
// КАЖДОЙ странице — шапка рисует его сама, а не получает слотом от страницы
// (`Header.astro`). С 2026-08-21 по эту дату здесь стояло обратное ожидание,
// а до 2026-08-21 — такое же, как сейчас.
test('на /thanks есть запасной канал и переключатель языка', async ({ page }) => {
  ...
  // Английской версии `/thanks` нет — переключатель ведёт на английскую
  // главную и говорит об этом подписью, а не молча в 404.
  const lang = page.locator('header a.lang');
  await expect(lang).toHaveCount(1);
  await expect(lang).toHaveAttribute('href', '/en');
});
