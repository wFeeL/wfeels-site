...
test.describe('приложение — находка брифа закрыта: «Замер» на главной больше не в фиксированном поле 384', () => {
  /* Раздел 1 брифа `12-case-pages-brief.md`: главная в его «поверхность» не
   * входила, и красное число здесь было ПОДТВЕРЖДЕНИЕМ находки, а не
   * требованием — до задачи, закрытой этой правкой (карточка «Кейсы»,
   * витрина раздела, выглядела недоделанной рядом с готовой версией самого
   * себя на `/cases/site-v3`, реестр 12.5). `.field` вокруг иллюстрации
   * «Замер» нёс ФИКСИРОВАННУЮ высоту 384/416 px
   * (`ILLUSTRATION_HEIGHT['site-v3']`/`SITE_V3_MOBILE_HEIGHT` в
   * `Cases.astro`, обе сняты этой правкой), рисунок в неё не был растянут, и
   * под готовым выводом («N× / в N раз легче») оставался пустой запас —
   * заполнение оттого было ниже порога. `CaseIllustrationField` получил тот
   * же авторазмер под содержимое, что панель разворота страницы кейса
   * (`CaseSpread.astro`, `.frame.inset`, П-3 того же брифа) уже несла —
   * раздел 4.5 брифа переносил рисунок туда отдельной задачей, эта задача
   * подтягивает главную к тому же устройству, а не изобретает новое.
   *
   * Замер после правки (см. лог задачи): 68,999 % на 1440 px, 65,327 % на
   * 390 px — оба выше порога 60 %. Тест перевёрнут на требование остаться
   * выше порога: если однажды заполнение снова уйдёт ниже — это будет
   * регрессия текущей задачи, а не повторное открытие старой находки. */
  test.use({ reducedMotion: 'reduce' });
  const ILLO = '[data-illustration="case-weight"]';

  for (const width of [1440, 390]) {
    test(`поле «Замера» на главной заполнено на ${width}px — авторазмер под содержимое`, async ({ page }) => {
      await page.setViewportSize({ width, height: 900 });
      await page.goto('/');
      ...
      console.log(
        `[case-spread-fill] «Замер» на главной @ ${width}px — заполнение ${(fresh.ratio * 100).toFixed(1)}%`,
      );
      expect(fresh.ratio).toBeGreaterThanOrEqual(PAINT_FILL_THRESHOLD);
    });
  }
