...
// Дизайн-ревью (мобильные дефекты, часть 2): семейства ссылок были меньше
// цели нажатия — ссылки услуг стояли без `min-height` (30 px, высота строки
// текста), а почтовая ссылка контакта приходит через `set:html` и не
// попадает под скоупленный CSS-селектор Astro (22 px, без единого локального
// правила). Два семейства — на разных секциях и разных механизмах поломки,
// поэтому проверяются одним тестом, а не заново растащены по существующим
// спекам секций.
//
// Ссылки ниш сюда больше не входят: правка владельца 2026-08-21 сняла
// строку ниш секции 3 («Под вашу отрасль: …») целиком — ссылок нет и в
// скором времени не будет, мерить цель для пальца стало нечего.
//
// Ссылки заголовков кейсов проверяются отдельно ниже: они вернулись только
// вместе с построенными detail-страницами.
test('на 390 px ссылки услуг и контакта пригодны для пальца',
  async ({ page }) => {
...
/* Заголовки ведут на detail-страницы, стрелки обеих галерей остаются
   отдельными органами управления и держат полную площадь касания. */
test('на 390 px ссылки кейсов и органы управления галереи пригодны для пальца', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/');
  ...
  const caseLinks = rows.locator('a.case-title-link');
  await expect(caseLinks).toHaveCount(count);
  for (let i = 0; i < count; i++) {
    const box = await caseLinks.nth(i).boundingBox();
    expect(box, `ссылка кейса ${i}`).not.toBeNull();
    expect(box!.height, `ссылка кейса ${i}: высота`).toBeGreaterThanOrEqual(TAP);
  }
