 ...
 * пять пунктов дают ≈900 px при контейнере 1060 px, шестой ломает строку».
 * Проверяется фактической шириной элемента, а не на глаз (план, задача 5).
 *
 * Контейнер (`tokens.css`, `--container: 1180px`) достигает объявленного
 * максимума при вьюпорте заметно шире 1060 px — 1440 выбран той же шириной,
 * на которой снимаются десктопные скриншоты приёмки. */
test.describe('первый экран — строка регалий не переносится при контейнере 1060 px', () => {
  test('пять пунктов стоят в одну строку, контейнер на объявленном максимуме', async ({ page }) => {
    ...
    expect(containerWidth, 'контейнер обязан быть на объявленном максимуме 1060 px')
      .toBeGreaterThan(1058);
