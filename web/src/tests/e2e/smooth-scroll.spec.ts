...
async function anchorLandings(page: Page) {
  ...
  for (const href of hrefs) {
    ...
    await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }));
    await page.waitForTimeout(150);
    /* Ссылка приводится в кадр ОТДЕЛЬНЫМ шагом, до клика, а не через
       автопрокрутку самого `.click()` (правка 2026-08-26, якоря — Lenis,
       `anchors: true`, `Base.astro`). Причина не косметика: Playwright перед
       кликом по невидимому элементу сам мгновенно доскролливает его в кадр
       (CDP), и это движение — НАСТОЯЩАЯ прокрутка окна — долетает до Lenis
       только СЛЕДУЮЩИМ событием `scroll`, асинхронно. Клик же диспетчерится
       сразу следом, в тот же тик: обработчик Lenis успевает посчитать цель
       ДО того, как его собственный `animatedScroll` пересинхронизировался с
       уже случившимся автоскроллом, и получает цель, заниженную ровно на
       величину этого автоскролла (замер `#guarantees`, 1440×900: автоскролл
       на 1066 px, посадка вместо 90 px пришла на 1156 px — ровно 1066 px
       мимо). Живой человек так кликнуть не может: клик всегда приходится на
       уже видимое, без слитного «скролл-и-клик-одним-тиком». Разделение
       шагов здесь и есть точное воспроизведение того, как кликает человек —
       не поблажка тесту. Проверено: тот же клик, но с паузой между наведением
       в кадр и самим кликом, приземляется на 90 px без единого промаха. */
    await page.locator(`a[href="${href}"]`).first().scrollIntoViewIfNeeded();
