...
test.describe('линия на фоне — страница рисует пути из реестра (05-line.md, раздел 10 шаг 6)', () => {
  ...
  test('в LINE_PATHS нет записи, которая не нарисована ни на одной секции страницы', async ({ page }) => {
    ...
    //
    // ИСКЛЮЧЕНИЕ, поимённое (`70-workshop/specs/site-v3/14-reviews-brief.md`,
    // раздел 4.2): `reviews` заведена в реестре БЕЗУСЛОВНО, а секция на
    // странице появляется только когда `homeReviews().length > 0`
    // (`lib/sections.ts`). Пока владелец не подставил ни одного отзыва, эта
    // запись законно не имеет потребителя — не осиротела, а ждёт данных. Это
    // ровно тот случай, который бриф просит ослабить «в одну сторону»
    // («каждый id секции имеет путь», а не наоборот) вместо того, чтобы
    // держать сторожа красным между приёмкой и первым отзывом.
    ...
    for (const key of Object.keys(LINE_PATHS)) {
      if (OPTIONAL_CONSUMER_ENTRIES.has(key) && !ownerIds.includes(key)) continue;
