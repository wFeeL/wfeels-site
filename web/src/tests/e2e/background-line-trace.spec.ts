...
test.describe('П-Э5 — список законных слепых прогонов полон и измерен числом', () => {
  test('1180/1440: боксы вне трёх целей, задетые своим траверсом, — известные прогоны, не шестой безымянный класс', async ({ browser }) => {
    for (const width of [1180, 1440] as const) {
      ...
      // cases: «Сайты» (websites, bare) несёт собственный непрозрачный
      // `.stage` — калька с него снята этой же правкой (WebsiteGallery.astro),
      // и траверс `cases` его тоже задевает (найдено этой правкой).
      const stage = await measureBlindRun(page, '#cases', '#cases .stage');
      // eslint-disable-next-line no-console
      console.log(`[П-Э5 @${width}px] cases .stage (галерея «Сайты»):`, JSON.stringify(stage));
