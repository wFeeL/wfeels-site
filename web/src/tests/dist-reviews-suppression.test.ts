...
describe('dist/index.html и dist/en/index.html — подавление секции «Отзывы»', () => {
  ...
  it(`точка рельса «ОТЗЫВЫ»/«REVIEWS» встречается ровно ${expectedCount} раз на каждой версии`, () => {
    // Рельс (`Rail.astro`) печатает точку только для секции, которая
    // существует, — точка помечена атрибутом `data-target="reviews"`
    // (уникален для точки рельса, не путается с меткой самой секции).
    //
    // ПРАВКА (проверка обоих состояний перед сдачей, раздел 4.12 брифа):
    // текстовый поиск `>ОТЗЫВЫ<`/`>REVIEWS<` был снят — секция несёт
    // СОБСТВЕННУЮ метку `.t-label` (`Section.astro`, `GAP.reviews.label`)
    // с тем же текстом, что и подпись рельса, и при непустом `REVIEWS`
    // строка «ОТЗЫВЫ» встречается в разметке ДВАЖДЫ (метка секции + точка
    // рельса), а не один раз — тот же приём уже несут все остальные точки
    // главной (замер: `ГАРАНТИИ`/`FAQ` тоже встречаются дважды каждая).
    // Старая формула `expectedCount` (0 или 1) с текстовым поиском ловила
    // бы это как ложное падение уже на первой временной записи — поймано
    // ИМЕННО той самой проверкой «оба состояния», которую требует раздел
    // 4.12, до того как это увидел бы владелец на боевой сборке.
    expect(countOccurrences(ruHtml, 'data-target="reviews"')).toBe(expectedCount);
    expect(countOccurrences(enHtml, 'data-target="reviews"')).toBe(expectedCount);
  });
