...
describe('dist/index.html — секции 1–3', () => {
  ...
  it('секция 2: метка, заголовок, все четыре строки боли и карточка-ответ', () => {
    ...
    expect(html).toContain('Смета растёт по ходу, и узнаёте вы об этом к сдаче');
    ...
    expect(html).toContain('Договорённость фиксируется до начала работы');
    expect(html).toContain('Чем это подтверждается — процесс и гарантии ниже');
  });
  ...
  it('секция 4: ступень 70 000 ₽ («Сайт до 10 страниц») не показана как карточка/цена в разметке', () => {
    // Внутри самой секции цен «70 000 ₽» законно встречается ровно ДВА раза —
    // оба раза как порог в прозе, не как карточка: во вводном абзаце про
    // условия оплаты («по вехам — для работ от…», правка владельца
    // 2026-08-13, часть 5) и в сноске про раунды правок («три для работ
    // от…»). Оба чтения — один и тот же порог `EDITS_THRESHOLD_PRICE`
    // (`home/Pricing.astro`): правки от 70 000 ₽ и оплата по вехам от
    // 70 000 ₽ по PRICING.md — одна и та же верхняя ступень «Сайты». Секция
    // вырезана по своим `id="pricing"…id="cases"`: та же строка легитимно
    // встречается и вне секции цен (условие рассрочки в «Гарантиях»), и это
    // не то, что проверяет этот тест.
    ...
    expect(occurrences, 'ступень 70 000 ₽ не должна повторяться карточкой внутри секции цен').toBe(2);
  });
