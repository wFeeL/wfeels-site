...
describe('английская главная — по-английски', () => {
  ...
  it('кириллицы на странице нет — кроме переключателя и официального ФИО', () => {
    ...
    /* Отзыв (`home/Reviews.astro`, `[data-review]`) — раздел 4.10 брифа
       `70-workshop/specs/site-v3/14-reviews-brief.md`: цитата не переводится
       НИКОГДА («перевод чужой речи без ведома автора — та же правка, что и
       обрезка») и несёт `lang="ru"` на `<blockquote>`; подпись (имя, род
       занятий, проект) — «как записаны», тоже без перевода, и `lang` на
       `<figcaption>` не ставится вовсе (только рамка секции — метка, `h2`,
       разделитель подписи — переведена). Русский текст ВНУТРИ `<figure
       data-review>` целиком снимается из проверки этим блоком, а не только
       `<blockquote>`: иначе цитата с `lang="ru"` прошла бы, а соседняя с ней
       нейтральная подпись без атрибута ловилась бы этим же тестом как
       нарушение — тот же текст, та же причина непереводимости, разный узел. */
    const withoutReviews = withoutSwitch.replace(
      /<figure\b[^>]*\bdata-review\b[^>]*>[\s\S]*?<\/figure>/g,
      '',
    );
    const cyrillic = [...withoutReviews.matchAll(/[^\n]*[А-Яа-яЁё][^\n]*/g)]
      .map((m) => m[0].trim())
      // Реквизиты не переводим: без подтвержденной транслитерации это было бы другое имя.
      .filter((text) => !text.includes('Сабуров Даниил Денисович'));
