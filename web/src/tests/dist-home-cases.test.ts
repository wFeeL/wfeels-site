...
describe('dist/index.html — секция 5', () => {
  ...
  /* Ссылка вернулась вместе с detail-страницами, поэтому сторож теперь
     требует точный адрес каждого опубликованного блока. Метка второй
     ссылкой не дублируется: единственный переход — подчёркнутый заголовок. */
  it('каждый блок ведет на свою построенную страницу ровно одной ссылкой', () => {
    const start = html.indexOf('id="cases"');
    const end = html.indexOf('id="process"');
    const section = html.slice(start, end);
    for (const c of homeCases()) {
      expect(section.match(new RegExp(`href=["']\\/cases\\/${c.slug}["']`, 'g')) ?? [], c.slug)
        .toHaveLength(1);
    }
