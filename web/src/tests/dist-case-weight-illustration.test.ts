...
describe('dist/index.html — иллюстрация «Замер»', () => {
  ...
  it('кратность несёт якорь счётчика, а фраза — нет', () => {
    // Правка владельца 2026-08-20: «добавляем такой же счетчик, как и для
    // других счетчиков в этом кейсе, для „6×“». Счётчик берёт всё, на чём
    // стоит `data-count`; потеря якоря вернула бы готовую кратность и была
    // бы молчаливой — сборка зелёная, разметка на месте, счёта нет.
    const verdict = cell(weightHtml, 'verdict');
    expect(verdict, 'кратность без якоря счётчика').toMatch(
      /<span class="mult"[^>]*\sdata-count[\s>]/,
    );
    // Слово считать нечем — оно выведено из кратности, а не написано
    // цифрами. Якорь на нём означал бы, что счётчик попытается разобрать
    // «в шесть раз легче» как число.
    const phrase = /<span class="phrase"[^>]*>/.exec(verdict)?.[0] ?? '';
    expect(phrase, 'на фразе не должно быть якоря счётчика').not.toContain('data-count');
  });
  ...
  // рисунке без названного канала. Проверка развёрнута в обратную: клетки
  // `data-cell="link"` на рисунке быть не должно, иначе снятие можно молча
  // откатить, и никто не заметит.
  it('клетки с оговоркой о канале на рисунке больше нет', () => {
    const link = cell(weightHtml, 'link');
    expect(link, 'клетка data-cell="link" обязана отсутствовать').toBe('');
    expect(weightHtml).not.toMatch(/ПОЛНАЯ ЗАГРУЗКА/i);
  });
  ...
  it('конечное состояние — состояние по умолчанию: ни одного нуля в разметке', () => {
    ...
    // Кратность — пятое считаемое число с 2026-08-20, и правило то же: в
    // разметке она стоит готовой, «0×» существует только в кадре счёта.
    const printedMult = /<span class="mult"[^>]*>([^<]+)</.exec(cell(weightHtml, 'verdict'))?.[1];
    expect(printedMult, 'кратность: напечатанное значение')
      .toBe(`${WEIGHT_ILLUSTRATION.multiplier}×`);
  });
