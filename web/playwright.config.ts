  ...
  webServer: [
    ...
    },
    {
      ...
      env: {
        ...
        SITE_URL,
        // Контейнер в бою получает `/data` и `/backups` как именованные
        // volumes. Локальный Playwright работает без Docker, поэтому использует
        // изолированную БД процесса и не пытается писать в корень macOS.
        LEADS_DB_PATH: ':memory:',
        BACKUP_DIRECTORY: `/tmp/wfeels-playwright-backups-${API_PORT}`,
