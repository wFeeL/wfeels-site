...
for (const page of PAGES) {
  ...
  const claim = html.match(/весит\s+(\d+)\s*КБ/);
  if (claim) {
    ...
    /* Кратность выведена из веса, поэтому проверяется тем же заходом.
       Медиана HTTP Archive — 2,5 МБ (`02-texts.md`, секция 6). */
    const WORDS = ['', '', 'два', 'три', 'четыре', 'пять', 'шесть', 'семь',
      'восемь', 'девять', 'десять', 'одиннадцать', 'двенадцать', 'тринадцать'];
    const ratio = Math.round((2.5 * 1024 * 1024) / total);
    const multiplier = html.match(/в\s+([а-яё]+)\s+раз\s+больше/i);
    if (multiplier && WORDS[ratio] && multiplier[1].toLowerCase() !== WORDS[ratio]) {
      console.error(
